...
module Critic::Functional
  class LogTest < Test
    ...
    describe 'generates a kv' do
      ...
      it 'log entry from info' do
        event = stub(:data => ["A Message", {:key1 => "ValueOne", :key2 => ["An", "Array"]}], :time => Time.new(1979,4,9), :level => 1)
        assert_equal("[1979-04-09 00:00:00.000000] pid=9973 level=\"info\" message=\"A Message\" bad=false key1=ValueOne key2=[\"An\",\"Array\"]
",
          layout.format(event)
        )
      end
      ...
      it 'log entry from error without a backtrace' do
        event = stub(:data => ["Another Message", StandardError.new], :time => Time.new(1979,4,9), :level => 3)
        assert_equal("[1979-04-09 00:00:00.000000] pid=9973 level=\"error\" message=\"Another Message\" bad=true error=StandardError error_class=StandardError
",
          layout.format(event)
        )
      end
      ...
      it 'log entry from error with a backtrace' do
        ...
        error.set_backtrace(backtrace)
        event = stub(:data => ["Yet Another Message", error], :time => Time.new(1979,4,9), :level => 3)
        assert_equal("[1979-04-09 00:00:00.000000] pid=9973 level=\"error\" message=\"Yet Another Message\" bad=true error=StandardError error_class=StandardError backtrace=
  a fake
  backtrace
",
          layout.format(event)
